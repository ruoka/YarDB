...
| Logic | `not`, `and`, `or` (precedence: `not` > `and` > `or`); `not` is `!match(P)` (not `$ne`/`$lte` rewrites); nested/double `not` restores positive comparison branches; expressions that expand to more than 64 OR branches are rejected |
