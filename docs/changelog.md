...
| Item | Summary |
|------|---------|
| **Prometheus `GET /metrics`** | HTTP metrics via `net::http::middleware::metrics_middleware` — `http_requests_total` and `http_request_duration_seconds` labeled by `method`, `status`, `path` (query stripped; digit-only segments → `{id}`), and `scenario` (`X-Metrics-Scenario` or `-`); public with PAT; scrapes not counted; unique series capped with `path="_other"` overflow |
