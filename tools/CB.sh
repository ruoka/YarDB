...
# Detect YarDB project structure and build include flags
# Get the current working directory (where cb will be invoked from)
CURRENT_DIR="$(pwd)"
INCLUDE_FLAGS=()

# Check if we're in the YarDB project root (has YarDB/ directory and deps/)
if [[ -d "$CURRENT_DIR/YarDB" && -d "$CURRENT_DIR/deps" ]]; then
    # YarDB project structure
    INCLUDE_FLAGS=(
        -I "$CURRENT_DIR/YarDB"
        -I "$CURRENT_DIR/deps/net/src"
        -I "$CURRENT_DIR/deps/xson/src"
        -I "$CURRENT_DIR/deps/cryptic"
        -I "$CURRENT_DIR/deps/cryptic/cryptic"
        -I "$CURRENT_DIR/deps/tester/tester"
        -I "/opt/homebrew/include"
    )
# Check if we're in a subdirectory of YarDB project
elif [[ "$CURRENT_DIR" == *"/YarDB"* ]] && [[ -d "$(cd "$CURRENT_DIR/../.." 2>/dev/null && pwd)/YarDB" ]]; then
    ...
    if [[ -n "$PROJECT_ROOT_FOUND" ]]; then
        INCLUDE_FLAGS=(
            -I "$PROJECT_ROOT_FOUND/YarDB"
            -I "$PROJECT_ROOT_FOUND/deps/net/src"
            -I "$PROJECT_ROOT_FOUND/deps/xson/src"
            -I "$PROJECT_ROOT_FOUND/deps/cryptic"
            -I "$PROJECT_ROOT_FOUND/deps/cryptic/cryptic"
